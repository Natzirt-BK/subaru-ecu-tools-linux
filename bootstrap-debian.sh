...
if [[ "${os_family,,}" != *debian* ]]; then
    echo "This bootstrap is intended for Debian and Debian-family distributions." >&2
    echo "Detected: ${PRETTY_NAME:-unknown}" >&2
    exit 1
fi
if [[ "$(uname -m)" != x86_64 ]]; then
    echo "This release requires Debian amd64; detected $(uname -m)." >&2
    exit 1
fi

if ! command -v git >/dev/null; then
    echo "Installing Git from the configured Debian repositories..."
    sudo apt-get update
...
if (($# == 0)); then
    exec "$source_dir/linux/setup-debian-gui.sh"
fi
if [[ "${1:-}" == --gui || "${1:-}" == --terminal ]]; then
    shift
    exec "$source_dir/linux/setup-debian-gui.sh" "$@"
fi
exec "$source_dir/linux/install-debian.sh" "$@"
